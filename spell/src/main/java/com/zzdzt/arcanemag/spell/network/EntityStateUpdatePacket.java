 ...
 *
 * 从 core 模块迁入 spell 模块：该包仅承载 spell 实体（ThunderStream / InkStrike）状态，
 * CHANNEL 切换至 SpellNetworking。
 */
public class EntityStateUpdatePacket {
    ...
    private static void handleThunderBeam(Entity entity, byte[] payload) {
        ...
        if (entity instanceof ThunderStreamEntity beam) {
    ...
    private static void handleInkStrike(Entity entity, byte[] payload) {
        ...
        if (entity instanceof InkThunderStrikeEntity strike) {
