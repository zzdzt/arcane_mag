...
public final class ManaMagazineLogic {
    ...
    /**
     * 指定等级下每发补弹的法力消耗。
     * {@link ArcaneMagConfig#MANA_MAGAZINE_BASE_COST} / {@code MANA_MAGAZINE_COST_REDUCTION_PER_LEVEL}
     * / {@code MANA_MAGAZINE_MIN_COST}，均可在配置中调整。
     */
    public static int costForLevel(int level) {
        int base = ArcaneMagConfig.MANA_MAGAZINE_BASE_COST.get();
        int reduction = ArcaneMagConfig.MANA_MAGAZINE_COST_REDUCTION_PER_LEVEL.get();
        int min = ArcaneMagConfig.MANA_MAGAZINE_MIN_COST.get();
